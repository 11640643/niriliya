...
class Goods extends Service
{
    ...
    public function getStatusConfig()
    {
        return [
            'status' => [
                'N' => $this->translate['put_off_shelves'],
                'Y' => $this->translate['being_put_on_shelves'],
                // 'O' => '金币不足'
            ],
        ];
    }
    ...
    public function apply($id, $uid,$num,$vip_level)
    {
        try{
            $num = abs(intval($num));
            if(empty($num) || $num  <= 0 ){
                throw new \Exception($this->translate['sys_abnormal_please_try_again']);
            }
            $goods = $this->di['s_goods']->search($id);            
            if(empty($goods) || $goods['status'] != 'Y'){
                throw new \Exception($this->translate['product_does_not_exist']);
            }
          
            if(empty($goods['vip_exchange'])){
                throw new \Exception($this->translate['abnormal_product_unit_place']);
            }

            $user_address = $this->di['s_address']->search(['uid'=>$uid, 'is_default' => 'Y']);
            if(empty($user_address)){
                throw new \Exception($this->translate['shipping_address_has_not_been_added']);
            }

            if (!$this->checkGoodsExchange($uid)) {
                throw new \Exception($this->translate['insufficient_exchange_credits']);
            }
            ...
            $add = [
                ...
                'num'=>$num,
                'thumb' => $goods['thumb'],
                'remark' => $this->translate['current_user_level_is'].$vip_level.' '.$goods['remark'],
            ...
            if(!$oid = $this->di['s_gorder']->save($add)){
                throw new \Exception($this->translate['failed_to_add_order']);
            }

            if (!$this->di['s_funds']->add($uid, -$total_money, 'fruit', 'goods_apply', "兑换:{$goods['name']}", $oid, 'Y', $numArray[0], $numArray[1])) {
                throw new \Exception($this->translate['failed_to_add_turnover']);
            }
