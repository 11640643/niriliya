...
class ReplyBack extends Service
{
    ...
    public function getStatusConfig()
    {
        return [
            'type' => [
                'A' => $this->translate['consultation'],
                'B' => $this->translate['complaint'],
                'C' => $this->translate['comment_suggestion'],
		        'D' =>$this->translate['others'],
            ],
        ];
    }
    ...
    public function reply($content,$reply_time,$reply_id)
    {
        try {
            ...
            if (!empty($reply_back['reply_content'])) {
                throw new \Exception($this->translate['already_replied']);
            }
	   #var_dump(['reply_content'=>$content,'reply_time'=>$reply_time]);exit;
		 ...
		 throw new \Exception($this->translate['reply_failed']);
	    }
	    return true;
