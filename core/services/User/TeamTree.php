...
class TeamTree extends Service
{
    ...
    public function calTeamTree($uid,$item_data,$cid)
    {
        ...
        $teaminfolist = $this->di['s_team']->searchAll();
        $_teaminfolist = [];
        foreach ($teaminfolist as $key => $value) {
           $_teaminfolist[$value['id']] = $value; 
        }
        // 等级列表
        $teaminfolist = $_teaminfolist;
        // 最高级别的等级
        $top_level_teaminfo = end($teaminfolist);
        ...
        foreach($list as $key => $val){
            ...
            $user_tmp['team_id'] = isset( $user_tmp['team_id'] )? $user_tmp['team_id']: 0;
            ...
            if ($key==0){
                foreach($children as $v){
                    ...
                    if($sumMoney >= $teaminfo['per_money']){
                        $people = count( $this->getAllTeamList($v['t_uid']));
                        // 如果是最高等级，跳过
                        if ( $top_level_teaminfo['id'] == $teaminfo['id'] ){
                            continue;
                        }
                        if ( count( $teaminfo) == 0 ){ $teaminfo['id']=0; }
                        //判断是否达到有效邀请人数。
                        if($people >=  $teaminfolist[$teaminfo['id']+1]['num']  ){  
                            $user_tmp = $this->di['s_user']->search($val['t_uid']);
                            //达到人数，直接升级
        ...
        // if ( $up_user_tmp['team_id']>0 ){
        //     foreach($teamlist as $val){
        //         if ( $val['id'] = ($up_user_tmp['team_id']-1)  ){
        //             print_r($val);
        //             print_R($val);
        //             exit();
        //             $addTeamData['il_id'] = $cid;
        //             $addTeamData['t_uid'] = $up_user_tmp['uid'];
        //             $addTeamData['team_apr'] = $val[$apr]/100;//计算出来的
        //             $addTeamData['team_apr_money'] = $addTeamData['money']/100;//计算出来的
        //             echo '<pre/>';
        //             print_r($addTeamData);
        //             exit();
