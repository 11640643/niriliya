...
class ItemList extends Service
{
    ...
    public function getStatusConfig()
    {
        return [
            'status' => [
                ...
                'Y' => $this->translate['new_lang_had'],
            ],
            'is_auto_apply' => [
                'N' =>$this->translate['new_lang_connect_open'],
                'Y' => $this->translate['new_lang_connect_close'],
            ],
            'type' => [
            ...
            'stype' => [
                'dq' => $this->translate['add_lang_short_item_financing'],
                'dt' => $this->translate['add_lang_fixed_item_financing'],
            ]
        ];
    }
    ...
    public function dqapply($uid, $money, $itemId, $passwd, $vouchers_money=0)
    {
        try {
            ...
            if (empty($item)) {
                throw new \Exception($this->translate['new_lang_project_closed']);
            }
            ...
            if ($item['rem_count']==0) {
                throw new \Exception($this->translate['new_project_sold_empty']);
            }
            ...
            if (!$cid = $this->save($addData)) {
                throw new \Exception($this->translate['new_lang_invest_error_1001']);
            }
            ...
            if ($item['pack'] > 0 && $money > 0 && $item['pack_max_num'] > 0) {
                ...
                if (!$this->di['s_funds']->add($uid, $packMoney, 'money', 'item_pack', $this->translate['new_lang_invest_hb'], $cid)) {
                    throw new Exception($this->translate['new_lang_flow_add_failed']);
                }
            ...
            if (!$this->di['s_iam']->saves($aprAddData)) {
                throw new \Exception($this->translate['new_lang_invest_error_1002']);
            }
            ...
            if ($item['money'] > 0) {
                ...
                if (!$this->di['s_item']->update($item['id'], ['schedule' => $schedule])) {
                    throw new \Exception($this->translate['new_lang_update_project_info_failed']);
                }
            }

            if ($item['prize_num'] > 0) {
                if (!$this->di['s_funds']->add($uid, $item['prize_num'], 'prize_num', 'prize_num_add_item', $this->translate['new_invset_se'], $cid)) {
                    throw new \Exception($this->translate['new_lang_award_time_failed']);
                }
            }
            ...
            if ($reward['item_credit'] > 0) {
                ...
                if (!$this->di['s_funds']->add($uid, $credit, 'credit', 'item_credit', $this->translate['new_lang_send_level_score'], $cid)) {
                    throw new \Exception($this->translate['new_user_score_failed']);
                }
            }
            if ($reward['exchange_credit'] > 0) {
                $exchange_credit = bcmul($money / 100, $reward['exchange_credit'], 2);
                if (!$this->di['s_funds']->add($uid, $exchange_credit , 'exchange_credit', 'exchange_credit', '投资送兑换礼品积分', $cid)) {
    ...
    public function reset($id)
    {
        try {
            ...
            if (!$this->update($id, ['status' => 'Y', 'is_auto_apply' => 'N', 'is_delete' => 1])) {
                throw new \Exception($this->translate['new_lang_operation_order_failed']);
            }
